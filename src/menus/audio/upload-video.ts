...
class UploadVideo {
    ...
    public uploadVideo(files: FileList | File[]): void {
        ...
        // ------------------------------ 获取配置信息 ------------------------------
        // 服务端地址
        let uploadVideoServer = config.uploadAudioServer
        // 上传视频的最大体积，默认 1024M
        ...
        // 自定义上传视频的名称
        const uploadVideoName = config.uploadAudioName
        // 上传视频自定义参数
        const uploadVideoParams = config.uploadAudioParams
        // 自定义参数拼接到 url 中
        const uploadVideoParamsWithUrl = config.uploadAudioParamsWithUrl
        // 上传视频自定义 header
        const uploadVideoHeaders = config.uploadAudioHeaders
        // 钩子函数
        const uploadVideoHooks = config.uploadAudioHooks
        // 上传视频超时时间 ms 默认2个小时
        const uploadVideoTimeout = config.uploadAudioTimeout
        // 跨域带 cookie
        const withVideoCredentials = config.withAudioCredentials
        // 自定义上传
        const customUploadVideo = config.customUploadAudio
        // 格式校验
        const uploadVideoAccept = config.uploadAudioAccept
        ...
        if (uploadVideoServer) {
            ...
            const xhr = post(uploadVideoServer, {
                ...
                onTimeout: xhr => {
                    config.customAlert(t('上传音频超时'), 'error')
    ...
    public insertVideo(url: string): void {
        ...
        if (!config.customInsertVideo) {
            ...
            // @ts-ignore
            config.customInsertAudio(url)
            return
        }

        // 加载视频
        let video: any = document.createElement('audio')
        video.onload = () => {
            video = null
        }
        video.onerror = () => {
            config.customAlert(
                t('插入音频错误'),
                'error',
                `wangEditor: ${t('插入音频错误')}，${t('音频链接')} "${url}"，${t('下载音频失败')}`
            )
            video = null
        }
